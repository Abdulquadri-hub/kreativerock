...
if($res)
{
    ...
    $package = $package->getPackageInfo("id = '" . $packageid . "'");
    if(empty($package))
    {
    ...
    $payment = $transaction->makePayment($ref,$amount,$email,$phone,$customername,$currency,$skey,$packageid,$callbackUrl);
    ...
    if($paymentResponse['status'] === "success")    
    {
        ...
        $result = $purchase->registerPurchase($fields, $values);
        if($result)
        {
            $message = 'Purchase of ' . $qty . ' units from package ' . $package['packagename'];
            $status = "PENDING";
            $gateway = "FLUTTERWAVE";
            $paymentmethod = "CARD";
            ...
            $transaction = $transaction->registerTransaction($transactionFields, $transactionValues);
            if($transaction)
            {
                //
            }else {
                exit(json_encode(["status" => false, "code" => 500, "message" => "Transaction Not Successful."]));
            }
            ...
            exit(json_encode(["status" => false, "code" => 500, "message" => "Checkout Not Successful."]));
        }
