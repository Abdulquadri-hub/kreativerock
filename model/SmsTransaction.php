...
class SmsTransaction  {
    ...
    public function getSmsTransactionInfo($condition){
        return $this->model->findOne("sms_transactions", $condition);
    }
    public function checkIfSmsTransactionExists($condition){
        return count($this->model->findOne("sms_transactions", $condition)) > 0 ? true : false;
    }
    public function registerSmsTransaction($fields, $values){
        return $this->model->insertdata("sms_transactions", $fields, $values);
    }
    public function removeSmsTransaction($condition){
        return $this->model->deletedata("sms_transactions", $condition);
    }
    public function retrieveAllSmsTransaction($pageno, $limit){
        $data = $this->model->paginate("sms_transactions", " 1 ORDER BY id ASC", $pageno, $limit);
        return $data;
    }
    public function retrieveSmsTransactionByStatus($status, $pageno, $limit){
        $data = $this->model->paginate("sms_transactions", "status LIKE '$status' ORDER BY id ASC", $pageno, $limit);
        return $data;
    }    
    public function retrieveSmsTransactionByQuery($query, $pageno, $limit,$field = "*"){
        $data = $this->model->paginate("sms_transactions", $query, $pageno, $limit,$field);
        return $data;
    }    
    public function updateSmsTransactionDetails($query, $id){
        return $this->model->update('sms_transactions', $query, "WHERE id = $id");
    } 
