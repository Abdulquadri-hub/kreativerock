...
function ProceedToBulkSummary() {
    ...
    const template = `
    <div class="border-y py-3 flex flex-col gap-3 capitalize">
    ...
    <div class="pt-4"><button name="x" class="btn w-full !p-3">Continue to pay</button></div>
    `
    openModal({ template, title: 'Purchase Summary'})
    document.querySelector('[name="x"]')
    .addEventListener('click', () => redirect(`payment&module=${encodeURIComponent('sms')}&service=${encodeURIComponent('bulk sms')}&packagename=${ encodeURIComponent(selectedPackage.packagename)}&qty=${qtyInput.value}`))
}
