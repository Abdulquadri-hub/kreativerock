...
async function fetchprofiles(id = "") {
    ...
    if (request.status) {
        if (request) {
            ...
            const profilecard = document.querySelector('[name="profilecard"]')
            profilecard.children[0].src = request.imageurl ? `../../images/kyc/${request.imageurl}` : './images/default-avatar.png'
            profilecard.children[1].innerText = `${ request.firstname } ${ request.othernames[0] + '.'} ${ request.lastname }`
            profilecard.children[2].innerText = request.email
            profilecard.children[3].lastElementChild.innerText = request.role.toLowerCase()
