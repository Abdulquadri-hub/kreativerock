...
async function fetchSmsPackage(packageName, returnResponse=false) {
    ...
    let request = await httpRequest('../controllers/sms/fetchpackages', packageName ? payload : null, null, 'json')
    if(request.status) {
...
async function paymentSubmitHandler() {
    ...
    if(selected.getAttribute('name') == 'FlutterWave'){
        request =  await httpRequest('../controllers/sms/checkoutsms', payload,  payBtn)
