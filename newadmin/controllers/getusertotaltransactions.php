...
$result = $admin->executeByQuerySelector("SELECT count(*) AS transactionstotal FROM transactions WHERE status LIKE '$status' AND productid IN (SELECT id FROM products WHERE user = '$email')");
if($result){
    $ttotal = array(
